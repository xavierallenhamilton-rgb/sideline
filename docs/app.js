...
// ---------- approve action ----------
// Returns a short status string ("approved" | "cancelled" | "error: ...") so
// callers (including the assistant's tool-call flow) know the real outcome.
...
// ============================================================
// Assistant — a real conversational agent over an open-source
// model via OpenRouter (not Claude), with one real tool
// (approve_task) gated behind the same confirm dialog the
// manual Approve buttons use. Nothing is ever written silently.
// ============================================================
...
function buildSystemPrompt() {
  return [
    "You are the Sideline Mission Control voice/chat assistant, embedded in a live dispatch-board webpage for a small AI-agent business that finds no-website local businesses and pitches them a free demo site.",
...
async function callOpenRouter(messages) {
  ...
  const res = await fetch(OPENROUTER_URL, {
    ...
    headers: {
      ...
      "X-Title": "Sideline Mission Control",
    },
    body: JSON.stringify({
      model: getAiModel(),
      messages,
